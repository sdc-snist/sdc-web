    ...
    <div className={`border-l-8 p-6 rounded-xl shadow-md ${cardColor}`}>
      <div className="flex justify-between items-center">
        ...
        <div className="text-right">
          ...
          {speaker && (
            <p className="text-lg text-red-600 italic">by {speaker}</p>
          )}
        </div>
      </div>
    </div>
...
function AIagent() {
  ...
  const schedule = [
    ...
    {
      time: "2:00 PM",
      title: "Hands-on: Multi-Agent Systems",
      speaker: "Priya Patel",
      type: "workshop",
    },
    ...
    <div className="min-h-screen bg-gradient-to-br from-red-50 to-white">
      <header className="bg-white shadow-sm border-b-2 border-red-500">
        <div className="max-w-7xl mx-auto px-6 py-6 flex items-center justify-between">
          ...
          <span className="bg-red-600 text-white px-4 py-2 rounded-full text-lg font-semibold">
            Limited Seats
          </span>
        </div>
      </header>
      ...
      <section className="bg-red-700 text-white py-24 text-center">
        ...
        <p className="mt-6 text-2xl">
          Join us for a hands-on AI Agents workshop at SNIST Campus
        </p>
        <div className="mt-8 flex flex-col sm:flex-row justify-center gap-6 text-lg">
          <div className="flex items-center justify-center gap-3">
            <Calendar className="w-6 h-6" /> August 10, 2025
          </div>
          <div className="flex items-center justify-center gap-3">
            <MapPin className="w-6 h-6" /> SNIST, Hyderabad
          </div>
          <div className="flex items-center justify-center gap-3">
            <Users className="w-6 h-6" /> Only 50 seats
          </div>
        </div>
      </section>
      ...
      <section className="bg-red-600 text-white py-12 text-center">
        ...
        <div className="flex justify-center space-x-6">
          {Object.entries(timeLeft).map(([unit, value]) => (
            <div
              key={unit}
              className="bg-white text-red-600 rounded-xl p-6 min-w-[100px]"
            >
      ...
      <section className="bg-white py-20 text-center">
        ...
        <p className="text-lg text-gray-600 mb-8">
          Register now and build real-world AI systems
        </p>
        <button
      ...
      <footer className="bg-gray-900 text-white py-16 text-center">
        <img
          src={sdcLogo}
          alt="SDC"
          className="mx-auto h-16 w-16 rounded-full mb-6"
        />
